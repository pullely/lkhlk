...
Your backend holds one new-proj API key and **zero GitHub credentials**.
...
const newproj = new newproj({
  baseUrl: process.env.LKHLK_API_URL!,
  token: process.env.LKHLK_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await newproj.integrations.issueGithubToken(ORG_ID, {
