...
Part of the lkhlk runtime: a Cloudflare Worker deployed per environment (`stage`, `prod`; `dev` is verify-only). Not publicly routable — reached only through `api-edge` service bindings.
...
- **web-console-next** — Next.js 15 + opennextjs/cloudflare delivery of the new-proj web console (per-environment, Workers + Static Assets)
