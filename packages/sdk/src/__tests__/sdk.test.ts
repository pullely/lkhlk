...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new newproj({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => newprojError]> = [
  ...
  it("forward-compat: unknown error code → base newprojError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(newprojError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as newprojError).code).toBe("future_quota_exceeded");
    }
  });
