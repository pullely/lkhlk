// Typed error hierarchy for the newproj SDK.
//
...
// codes (forward compatibility) fall back to the generic `newprojError`
...
export interface newprojErrorInit {
 ...
 * `quota_exceeded`) decode to this class — `instanceof newprojError`
 * still matches.
 */
export class newprojError extends Error {
  ...
  constructor(init: newprojErrorInit) {
    super(init.envelope.message);
    this.name = "newprojError";
...
export class BadRequestError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export class UnauthenticatedError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export class ForbiddenError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export class NotFoundError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export class ConflictError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export class ValidationError extends newprojError {
...
export class PreconditionFailedError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export class UnsupportedError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export class InternalError extends newprojError {
  constructor(init: newprojErrorInit) {
    super(init);
...
export interface RateLimitErrorInit extends newprojErrorInit {
...
export class RateLimitError extends newprojError {
...
/**
 * Decode an HTTP `Response` into the appropriate `newprojError` subclass.
 *
 * Forward-compatible: unknown error codes resolve to the base `newprojError`.
...
export async function decodeError(
  ...
  switch (envelope.code as ErrorCode) {
    ...
    default:
      ...
      return new newprojError(init);
  }
}

function decodeRateLimit(
  init: newprojErrorInit,
  response: Response,
): RateLimitError {
