// Base HTTP transport for the newproj SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `newprojError`
...
export interface ClientOptions {
  /**
   * Base URL of the newproj api-edge worker, e.g.
   * `https://api.asa.com`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `newproj` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "newproj SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "newproj SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
