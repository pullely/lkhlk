...
new-proj TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the new-proj control plane API.

## Install (workspace)

This package is internal to the `lkhlk` monorepo. Add it as a
...
```ts
import { newproj } from "@saas/sdk";

const client = new newproj({
  baseUrl: "https://api.asa.com",
  auth: { kind: "bearer", token: process.env["LKHLK_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `newprojError`. Branch on
...
| `newprojError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `newprojError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new newproj({ fetch })`.
