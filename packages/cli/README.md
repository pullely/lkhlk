...
`lkhlk` — first-class TypeScript CLI for the new-proj control
...
This package is internal to the `lkhlk` monorepo and is not
...
```
lkhlk login    [--api-url=URL] [--token=BEARER]
lkhlk logout
lkhlk whoami
```
...
```
lkhlk org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
lkhlk project create <name> [--idempotency-key=KEY]
lkhlk env create <project-id> <name> [--idempotency-key=KEY]
lkhlk api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
lkhlk webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
lkhlk usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
lkhlk billing summary
lkhlk audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`lkhlk org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `lkhlk login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/lkhlk/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/lkhlk/config.json` (mode 0644, not a secret). Override
both via `LKHLK_CONFIG_DIR` (used by tests).
