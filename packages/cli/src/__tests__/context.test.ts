...
describe("resolveConfigDir", () => {
  it("respects LKHLK_CONFIG_DIR override", () => {
    const prev = process.env["LKHLK_CONFIG_DIR"];
    process.env["LKHLK_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["LKHLK_CONFIG_DIR"];
      else process.env["LKHLK_CONFIG_DIR"] = prev;
    }
  });
});
